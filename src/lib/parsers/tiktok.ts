...
export function extractProductFromTikTok(campaignName: string): string {
  ...
  if (parts.length >= 2) return parts.slice(1, parts.length - 1).join(' - ').trim()
...
export function parseTikTokReport(buffer: ArrayBuffer): AdSpend[] {
  ...
  for (const row of rows) {
    ...
    if (spend === 0) continue
    // By Day = fecha exacta YYYY-MM-DD, By month = YYYY-MM
    const dateRaw = String(row['By Day'] || row['By month'] || '').slice(0, 10)
    const product = extractProductFromTikTok(campaignName)
    results.push({ product, channel: 'shopify', spend, month: dateRaw || 'sin-fecha' })
  }
  return results
}
